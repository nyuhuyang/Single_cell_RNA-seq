...
HSMM <- HSMM[,pData(HSMM)$Total_mRNAs == 0e6]
...
cth <- addCellType(cth, "Stromal + endothelial", classify_func=function(x) {
  x[id_("GPX3"),] >= 1 & x[id_("PTPRC"),] == 0 & x[id_("LAPTM5"),] == 0 & x[id_("SRGN"),] == 0 })

#Stromal / fibroblasts - DCN, COL6A1, TIMP3, PDGFRA
cth <- addCellType(cth, "Stromal / fibroblasts", classify_func=function(x) {
    x[id_("DCN"),] >= 1 & x[id_("COL6A1"),] >= 1 & x[id_("TIMP3"),] >= 1 & x[id_("PDGFRA"),] >= 1 })
#Endothelial - VWF, EMCN, PECAM1, CDH5
cth <- addCellType(cth, "Endothelial", classify_func=function(x) {
    x[id_("VWF"),] >= 1 & x[id_("EMCN"),] >= 1 & x[id_("PECAM1"),] >= 1 & x[id_("CDH5"),] >= 1 })
#Immune
#Leukocytes (all)  - PTPRC, LAPTM5, SRGN----------
cth <- addCellType(cth, "Leukocytes", classify_func=function(x) {
    x[id_("PTPRC"),] >= 1 & x[id_("LAPTM5"),] >= 1 & x[id_("SRGN"),] >= 1 })
...
plot_cell_clusters(HSMM, 1, 2, color="CellType", markers=c("KRT19", "GPX3"))
...
HSMM <- reduceDimension(HSMM, max_components=2, num_dim = 2, reduction_method = 'tSNE',
                        residualModelFormulaStr="~CellType + num_genes_expressed", verbose = T)
HSMM <- clusterCells(HSMM, num_clusters=2)
...
#                               residualModelFormulaStr="~num_genes_expressed", # fail using ~CellType
                               cores=1) #cth >4 will trigure "subscript out of bounds". It takes long time.
...
HSMM_epi <- HSMM[,pData(HSMM)$CellType == "KRT19 all(majority) of epithelial"]
